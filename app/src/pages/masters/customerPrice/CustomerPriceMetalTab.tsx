...
// ── Types ─────────────────────────────────────────────────────
// A price sheet is a customer header with many price lines. The grid lists the
// headers; the full-page form edits one header and its whole line set.
interface CustomerPriceMetal {
  id:                     number
  customer_id:            number
...
interface MetalLine {
  ...
  uom:                    string
  rhodium_amt:            number | string
  tricolor_rhodium_amt:   number | string
  lobster_amt:            number | string
  silky_rope_amt:         number | string
  hallmark_amt:           number | string
  remarks:                string | null
}
...
type LineForm = {
  ...
  uom:                    string
  rhodium_amt:            string
  tricolor_rhodium_amt:   string
  lobster_amt:            string
  silky_rope_amt:         string
  hallmark_amt:           string
  remarks:                string
}
...
const blankLine: LineForm = {
  itemtype: '', sku_code: '', applyAllSku: false, sales_group_code: '',
  rate_basis: 'PER_GM', rate_type: 'AMOUNT', rate_value: '',
  uom: 'GM',
  rhodium_amt: '', tricolor_rhodium_amt: '', lobster_amt: '', silky_rope_amt: '', hallmark_amt: '',
  remarks: '',
}
...
// Order-line input styling, shared by the plain cells and the pickers
...
// Item Type → LOV type for /fg-bom/lov/:type (same mapping Supplier Rate Contract uses)
...
function downloadCSVTemplate() {
  const headers = ['customer_code', 'sales_group_code', 'itemtype', 'sku_code', 'rate_basis', 'rate_type', 'rate_value', 'uom', 'rhodium_amt', 'tricolor_rhodium_amt', 'lobster_amt', 'silky_rope_amt', 'hallmark_amt', 'remarks']
  const example  = ['CUST-000001', 'RETAIL', 'FINDING', 'FIN-LOBSTER-18K-YG', 'PER_GM', 'AMOUNT', '1250.00', 'GM', '150.00', '225.00', '60.00', '75.00', '45.00', '']
  const csv = [headers.join(','), example.join(',')].join('\n')
  const blob = new Blob([csv], { type: 'text/csv' })
  const url  = URL.createObjectURL(blob)
  const a    = document.createElement('a')
  a.href = url; a.download = 'customer_price_metal_import_template.csv'
...
const CustomerPriceMetalTab: React.FC<TabProps> = ({ onFormOpen }) => {
  // ── LOV state ───────────────────────────────────────────────
  const [itemTypeOpts,   setItemTypeOpts]   = useState<LookupOption[]>([])
  const [salesGroupOpts, setSalesGroupOpts] = useState<LookupOption[]>([])
  ...
  useEffect(() => {
    ...
    apiService.get('/common/lookup/GROUP_SALES')
      .then(r => setSalesGroupOpts(r.data?.data ?? []))
  ...
  // ── Grid state ──────────────────────────────────────────────
  const [items,        setItems]        = useState<CustomerPriceMetal[]>([])
  ...
  const [selectedRows,      setSelectedRows]      = useState<CustomerPriceMetal[]>([])
  ...
  const [editItem,    setEditItem]    = useState<CustomerPriceMetal | null>(null)
  const [toggleItem,  setToggleItem]  = useState<CustomerPriceMetal | null>(null)
  ...
  const isRealLine = (l: LineForm) => !!(l.itemtype || l.sku_code || l.rate_value)

  const validate = (): boolean => {
    if (!validateHeader()) return false

    const real = lines.filter(isRealLine)
    if (real.length === 0) { toast.error('Add at least one price line'); return false }

    const seen = new Set<string>()
    for (let i = 0; i < lines.length; i++) {
      const l = lines[i]
      if (!isRealLine(l)) continue
      if (!l.itemtype) { toast.error(`Line ${i + 1}: Item Type is required`); return false }
      if (!l.sku_code) { toast.error(`Line ${i + 1}: SKU Code is required`); return false }

      const key = `${l.itemtype}|${l.sku_code}`
      if (seen.has(key)) { toast.error(`Line ${i + 1}: ${l.sku_code} is already priced on another line`); return false }
      seen.add(key)

      const numFields: (keyof LineForm)[] = ['rate_value', 'rhodium_amt', 'tricolor_rhodium_amt', 'lobster_amt', 'silky_rope_amt', 'hallmark_amt']
      for (const f of numFields) {
        const v = l[f] as string
        if (v !== '' && Number(v) < 0) { toast.error(`Line ${i + 1}: ${f.replace(/_/g, ' ')} cannot be negative`); return false }
      }
    }
    return true
  }
  ...
  // ── Item picker per line ──────────────────────────────────────
  // No Sales Group filter any more: Sales Group is now read off whichever item
  // is picked, so it cannot also narrow the list the item is picked from.
  const searchSkuLov = async (q: string, itemType: string): Promise<LovOpt[]> => {
    const lovType = LOV_BY_ITEM_TYPE[itemType]
    if (!lovType) return []
    const r = await apiService.get(`/fg-bom/lov/${lovType}`, { params: { search: q } })
    return r.data?.data ?? []
  }

  // Changing Item Type invalidates the SKU picked from the previous master, and
  // the Sales Group that came with it.
  const handleItemTypeChange = (idx: number, newType: string) => {
    setLines(ls => ls.map((l, i) => i === idx
      ? { ...l, itemtype: newType, sku_code: l.applyAllSku ? 'ALL' : '', sales_group_code: '' }
      : l))
  }

  // Picking an item fills Sales Group from that item's master record (FG and
  // Finding carry one on the variant; other item types have none, and the cell
  // stays open for a manual pick).
  const handleSkuSelect = (idx: number, code: string, opt: LovOpt) => {
    const group = String((opt.group_sales as string | null) ?? '').trim()
    setLines(ls => ls.map((l, i) => i === idx
      ? { ...l, sku_code: code, sales_group_code: group || l.sales_group_code }
      : l))
  }

  const handleSkuClear = (idx: number) => setLineField(idx, 'sku_code', '')
  ...
  const handleRateBasisChange = (idx: number, val: 'PER_GM' | 'PER_PC') => {
    setLines(ls => ls.map((l, i) => i === idx
      ? { ...l, rate_basis: val, uom: val === 'PER_PC' ? 'PCS' : (l.uom === 'PCS' ? 'GM' : l.uom) }
      : l))
  }

  const handleApplyAllSkuToggle = (idx: number, checked: boolean) => {
    setLines(ls => ls.map((l, i) => i === idx
      ? { ...l, applyAllSku: checked, sku_code: checked ? 'ALL' : '' }
      : l))
  }
  ...
  const loadStats = async () => {
    try {
      const res = await apiService.get('/customer-price/metal/stats')
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      const res = await apiService.get('/customer-price/metal', {
        params: {
      ...
      toast.error(msg || 'Failed to load customer price (metal) records')
    } finally {
  ...
  const toggleSelectRow = (item: CustomerPriceMetal) =>
  ...
  // ── Export ────────────────────────────────────────────────────
  const buildExportRows = (rows: CustomerPriceMetal[]) =>
    rows.map(r => ({
  ...
  const handleExport = async (scope: 'all' | 'selected', format: 'csv' | 'excel' | 'pdf') => {
    ...
    const fname = `customer_price_metal_${statusFilter}`
    if (format === 'csv')   exportToCSV(data, fname)
    else if (format === 'excel') void exportToExcel(data, fname)
    else void exportToPDF(data, fname, 'Customer Price Master (Metal) Report')
  }

  // ── Cell renderer ─────────────────────────────────────────────
  const renderCell = (col: ColDef, row: CustomerPriceMetal) => {
    switch (col.key) {
  ...
  const openSheet = async (item: CustomerPriceMetal, mode: 'edit' | 'view') => {
    try {
      const res = await apiService.get(`/customer-price/metal/${item.id}`)
      ...
      const dl: MetalLine[] = d.lines ?? []
      setLines(dl.map(l => ({
        itemtype:             l.itemtype ?? '',
        sku_code:             l.sku_code ?? '',
        applyAllSku:          l.sku_code === 'ALL',
        sales_group_code:     l.sales_group_code ?? '',
        rate_basis:           l.rate_basis ?? 'PER_GM',
        rate_type:            l.rate_type ?? 'AMOUNT',
        rate_value:           String(l.rate_value ?? ''),
        uom:                  l.uom ?? 'GM',
        rhodium_amt:          String(l.rhodium_amt ?? ''),
        tricolor_rhodium_amt: String(l.tricolor_rhodium_amt ?? ''),
        lobster_amt:          String(l.lobster_amt ?? ''),
        silky_rope_amt:       String(l.silky_rope_amt ?? ''),
        hallmark_amt:         String(l.hallmark_amt ?? ''),
        remarks:              l.remarks ?? '',
      })))
      setFormErrors({})
      setView('form')
  ...
  const openEdit = (item: CustomerPriceMetal) => { void openSheet(item, 'edit') }
  const openView = (item: CustomerPriceMetal) => { void openSheet(item, 'view') }
    ...
    try {
      const payload = {
        ...
        lines: lines.filter(isRealLine).map(l => ({
          itemtype:             l.itemtype,
          sku_code:             l.sku_code,
          sales_group_code:     l.sales_group_code || null,
          rate_basis:           l.rate_basis,
          rate_type:            l.rate_type,
          rate_value:           num(l.rate_value),
          uom:                  l.uom,
          rhodium_amt:          num(l.rhodium_amt),
          tricolor_rhodium_amt: num(l.tricolor_rhodium_amt),
          lobster_amt:          num(l.lobster_amt),
          silky_rope_amt:       num(l.silky_rope_amt),
          hallmark_amt:         num(l.hallmark_amt),
          remarks:              l.remarks.trim() || null,
        })),
      }
      if (isNew) {
        await apiService.post('/customer-price/metal', payload)
        toast.success('Customer price (metal) created successfully')
      } else {
        await apiService.put(`/customer-price/metal/${editItem!.id}`, payload)
        toast.success('Customer price (metal) updated successfully')
      }
      backToList()
  ...
  const confirmToggle = async (reason?: string) => {
    ...
    try {
      const res = await apiService.delete(`/customer-price/metal/${toggleItem.id}`, reason ? { data: { reason } } : undefined)
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await apiService.post('/customer-price/metal/import', { rows })
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between gap-2 text-xs" style={{ color: 'var(--text-muted)' }}>
          <div className="flex items-center gap-2">
            ...
            <span style={{ color: 'var(--accent-gold)' }}>
              {isNew ? 'New Metal Price' : editItem?.customer_company_name}
            </span>
          </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <h1 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>
            {isNew
              ? 'Add Metal Price'
              : `${isViewMode ? 'View' : 'Edit'} Metal Price — ${editItem?.customer_company_name ?? ''}`}
          </h1>
          {isViewMode && (
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-xs px-2 py-0.5 rounded-full bg-[var(--bg-secondary)] border border-[var(--border-color)]" style={{ color: 'var(--text-muted)' }}>
                Sales Group is filled from the picked item
              </span>
            </div>
            {!isViewMode && (
                  ...
                  {lines.map((l, idx) => (
                    <tr key={idx} className="border-t border-[var(--border-color)]">
                      ...
                      <td className="px-1 py-1.5" style={{ minWidth: '120px' }}>
                        <select value={l.itemtype} onChange={e => handleItemTypeChange(idx, e.target.value)}
                      ...
                      {/* SKU picker, or the ALL marker when this line prices every
                          SKU of its item type */}
                      <td className="px-1 py-1.5" style={{ minWidth: '190px' }}>
                        {l.applyAllSku ? (
                          <div className={`${lineInputCls} flex items-center bg-[var(--bg-tertiary)]`} style={{ color: 'var(--text-secondary)' }}>
                            ALL — every SKU of this item type
                          </div>
                        ) : (
                          <LineLovSearch
                            value={l.sku_code}
                            placeholder={l.itemtype ? 'Type or % for all…' : 'Select Item Type first'}
                            disabled={isViewMode || !l.itemtype}
                            fetch={q => searchSkuLov(q, l.itemtype)}
                            onSelect={(code, opt) => handleSkuSelect(idx, code, opt)}
                            onClear={() => handleSkuClear(idx)}
                          />
                        )}
                        {!isViewMode && (
                          <label className="flex items-center gap-1 mt-1 text-[10px] cursor-pointer select-none" style={{ color: 'var(--text-muted)' }}>
                            <input type="checkbox" checked={l.applyAllSku}
                              onChange={e => handleApplyAllSkuToggle(idx, e.target.checked)}
                              disabled={!l.itemtype}
                      ...
                      <td className="px-1 py-1.5" style={{ minWidth: '130px' }}>
                        <select value={l.sales_group_code} onChange={e => setLineField(idx, 'sales_group_code', e.target.value)}
                          disabled={isViewMode} className={lineInputCls} style={lineInputStyle}>
                          <option value="">—</option>
                          {l.sales_group_code && !salesGroupOpts.some(o => o.lookup_code === l.sales_group_code) && (
                            <option value={l.sales_group_code}>{l.sales_group_code}</option>
                          )}
                          {salesGroupOpts.map(o => (
                            <option key={o.lookup_code} value={o.lookup_code}>{o.lookup_name}</option>
                          ))}
                        </select>
                      </td>
                      ...
                      {([
                        ['rhodium_amt',          '90px'],
                        ['tricolor_rhodium_amt', '110px'],
                        ['lobster_amt',          '90px'],
                        ['silky_rope_amt',       '95px'],
                        ['hallmark_amt',         '90px'],
                      ] as const).map(([key, w]) => (
                        <td key={key} className="px-1 py-1.5" style={{ minWidth: w }}>
                          <input type="number" step="0.01" min="0" value={l[key]}
  ...
  return (
    <div>
        ...
        <div className="flex flex-wrap items-center gap-3 p-4 border-b border-[var(--border-color)]">
          ...
          <div className="relative flex-1 min-w-[180px] max-w-xs">
            ...
            <input
              ...
              placeholder="Search metal prices…"
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <div ref={colPickerRef} className="relative">
              ...
              {showColPicker && (
                <div className="absolute right-0 top-full mt-1 z-50 w-52 rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] shadow-lg p-1.5 max-h-72 overflow-y-auto">
                  ...
                  >
                    <div className="flex flex-col items-center gap-3">
                      ...
                      {search || Object.values(colFilters).some(v => v)
                        ? 'No metal price entries match the current filters.'
                        : `No ${statusFilter} metal price entries found. Click "Add Metal Price" to get started.`
                      }
                    </div>
                  </td>
                </tr>
      ...
      <Modal
        ...
        title="Import Metal Prices from CSV"
        size="xl"
        footer={
          <>
      ...
      <DeactivateReasonDialog
        ...
        title="Deactivate Metal Price"
        itemLabel={`Metal price for "${toggleItem?.customer_company_name}"`}
      ...
      <ConfirmDialog
        ...
        title="Activate Metal Price"
        message={`Activate metal price for "${toggleItem?.customer_company_name}"?`}
...
export default CustomerPriceMetalTab
