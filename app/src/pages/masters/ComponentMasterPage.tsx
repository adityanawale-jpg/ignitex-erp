...
const schema = yup.object({
  ...
  component_name: yup.string().required('Component Name is required'),
  component_desc: yup.string().required('Component Description is required'),
  component_code: yup.string().default(''),
})
...
const INITIAL_COLS: ColDef[] = [
  { key: 'component_code', label: 'Component Code', sortKey: 'component_code', visible: true, minW: '180px' },
  { key: 'component_type', label: 'Component Type', sortKey: 'component_type', visible: true, minW: '140px' },
  { key: 'component_name', label: 'Component Name', sortKey: 'component_name', visible: true, minW: '140px' },
  { key: 'component_desc', label: 'Description',    sortKey: 'component_desc', visible: true, minW: '140px' },
...
// ── LOV types ─────────────────────────────────────────────────
const LOV_TYPES = ['COMPONENT_TYPE', 'COMPONENT_NAME', 'COMPONENT_DESC'] as const
type LovType = typeof LOV_TYPES[number]

const FIELD_LOV: Record<keyof Omit<FormValues, 'component_code'>, LovType> = {
...
// ── Component ─────────────────────────────────────────────────
const ComponentMasterPage: React.FC = () => {
  ...
  const [sortBy,       setSortBy]       = useState('component_code')
  ...
  const watchType = watch('component_type')
  const watchName = watch('component_name')
  const watchDesc = watch('component_desc')

  // Live preview of code in add mode — sequence number shown as ???
  useEffect(() => {
    if (!isNew) return
    if (watchType && watchName && watchDesc) {
      setValue('component_code', `${watchType}-${watchName}-${watchDesc}-???`.toUpperCase())
    } else {
      setValue('component_code', '')
    }
  }, [watchType, watchName, watchDesc, isNew, setValue])
  ...
  const loadStats = async () => {
    try {
      const res = await apiService.get('/component-master/stats')
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      const res = await apiService.get('/component-master', {
        params: {
      ...
      toast.error(msg || 'Failed to load components')
    } finally {
  ...
  const buildExportRows = (rows: ComponentItem[]) =>
    rows.map(r => ({
      'Component Code': r.component_code,
      'Component Type': lovName('COMPONENT_TYPE', r.component_type),
      'Component Name': lovName('COMPONENT_NAME', r.component_name),
      'Description':    lovName('COMPONENT_DESC', r.component_desc),
  ...
  const handleExport = async (scope: 'all' | 'selected', format: 'csv' | 'excel' | 'pdf') => {
    ...
    const fname = `component_master_${statusFilter}`
    if (format === 'csv')        exportToCSV(data, fname)
    else if (format === 'excel') void exportToExcel(data, fname)
    else                         void exportToPDF(data, fname, 'Component Master Report')
  }
  ...
  const renderCell = (col: ColDef, row: ComponentItem) => {
    switch (col.key) {
      case 'component_code':
        return <span className="font-mono text-sm font-semibold" style={{ color: 'var(--accent-gold)' }}>{row.component_code}</span>
      case 'component_type':
        return <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>{lovName('COMPONENT_TYPE', row.component_type)}</span>
      case 'component_name':
        return <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>{lovName('COMPONENT_NAME', row.component_name)}</span>
      case 'component_desc':
        return <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>{lovName('COMPONENT_DESC', row.component_desc)}</span>
      case 'is_active':
  ...
  // ── Modal helpers ─────────────────────────────────────────────
  const emptyForm: FormValues = { component_type: '', component_name: '', component_desc: '', component_code: '' }
  ...
  const onSubmit = async (data: FormValues) => {
    setSaving(true)
    try {
      const { component_code: _omit, ...payload } = data
      if (isNew) {
        await apiService.post('/component-master', payload)
        toast.success('Component created successfully')
      } else {
        await apiService.put(`/component-master/${editItem!.id}`, payload)
  ...
  const confirmToggle = async (reason?: string) => {
    ...
    try {
      const res = await apiService.delete(`/component-master/${toggleItem.id}`, reason ? { data: { reason } } : undefined)
  ...
  const LovSelect = ({
    ...
    field: keyof Omit<FormValues, 'component_code'>
    label: string
    required?: boolean
  }) => {
  ...
  return (
    <div>
                    ...
                    {visibleCols.map(col => (
                      <td key={col.key} className="px-4 py-3">
                        <div className="h-4 rounded animate-pulse bg-[var(--bg-tertiary)]"
                          style={{ width: col.key === 'component_code' ? '160px' : '100px' }} />
                      </td>
                    ))}
                  ...
                  <td colSpan={visibleCols.length + 2} className="px-4 py-16 text-center text-sm" style={{ color: 'var(--text-muted)' }}>
                    <div className="flex flex-col items-center gap-3">
                      ...
                      {search || Object.values(colFilters).some(v => v)
                        ...
                        : `No ${statusFilter} components found. Click "Add Component" to get started.`
                      }
                    </div>
                  </td>
                </tr>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-t border-[var(--border-color)] bg-[var(--bg-secondary)]">
          <div className="flex items-center gap-3">
            <span className="text-sm" style={{ color: 'var(--text-muted)' }}>
              {total === 0 ? 'No records found' : `Showing ${startRow}–${endRow} of ${total} components`}
            </span>
            {selectedRows.length > 0 && (
      ...
      <Modal
        ...
        title={
          isNew
            ? 'Add Component'
            : isViewMode
              ? `View Component — ${editItem?.component_code}`
              : `Edit Component — ${editItem?.component_code}`
        }
        size="lg"
        footer={
          isViewMode ? (
      ...
      >
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <LovSelect field="component_type" label="Component Type" required />
            <LovSelect field="component_name" label="Component Name" required />

            {/* Component Desc — full width */}
            <div className="sm:col-span-2">
              <LovSelect field="component_desc" label="Component Description" required />
            </div>

            {/* Component Code — always read-only, set by DB trigger */}
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>
                Component Code
                {isNew && (
          ...
          {isNew && (
            <div className="flex items-start gap-2 p-3 rounded-lg border border-[var(--accent-gold)]/30 bg-[var(--accent-gold)]/5">
              ...
              <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                Component Code is auto-generated as <strong>TYPE–NAME–DESC–NNN</strong> where NNN
                is a sequential number assigned by the system on save.
                The preview above shows <strong>???</strong> for the number — the actual code appears after saving.
              </p>
            </div>
          )}
        </div>
      ...
      <DeactivateReasonDialog
        ...
        itemLabel={`Component "${toggleItem?.component_code}"`}
      ...
      <ConfirmDialog
        ...
        message={`Activate component "${toggleItem?.component_code}"?`}
