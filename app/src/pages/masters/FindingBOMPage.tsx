...
function BOMLineGrid({ lines, setLines, readOnly, uomOpts, itemTypeOpts, purityOpts }: BOMLineGridProps) {
  ...
  const searchLov = async (q: string, itemType: string): Promise<LovOpt[]> => {
    ...
    const r = await apiService.get(`/fin-bom/lov/${lovType}?${params}`)
    return r.data?.data ?? []
  }
  ...
  const updateNetWt  = (v: number) => setEditing(p => p && ({ ...p, net_weight: v, gross_weight: v }))
  const updateCompWt = (v: number) => setEditing(p => p && ({ ...p, component_weight: v, gross_weight: v }))
              ...
              {filteredLines.map(l => (
                <tr key={l._key} className="border-t" style={{ borderColor: 'var(--border-color)' }}>
                  ...
                  <td className="px-3 py-2 text-xs text-right">
                    {l.item_type === 'METAL' ? fmt4(l.net_weight) : '—'}
                  </td>
                  <td className="px-3 py-2 text-xs text-right">
        ...
        <div className="p-5 rounded-xl border-2 space-y-4"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {!isM && fld('Qty', numIn(editing.item_quantity,
              v => setEditing(p => p && ({ ...p, item_quantity: v })),
              false, true, triedSave && isS && !editing.item_quantity),
              isS
            )}
            ...
            {isM && (
              <div>
                {fld('Net Wt (gm)', numIn(editing.net_weight, updateNetWt, false, false, triedSave && !editing.net_weight), true)}
              </div>
            )}

            {isC && fld('Component Wt (gm)', numIn(editing.component_weight, updateCompWt, false, false, triedSave && !editing.component_weight), true)}

            {isS && fld('Stone Cts',
              numIn(editing.stone_cts, updateStoneCts, false, false, triedSave && !editing.stone_cts),
              true
            )}
            {isS && fld('Stone Gm (auto)', numIn(editing.stone_gms, () => {}, true))}
...
export default function FindingBOMPage() {
  ...
  const netWeight      = useMemo(() =>
    +bomLines.filter(l => l.item_type === 'METAL')
  ...
  const loadStats = useCallback(async () => {
    if (view !== 'list') return
    try {
      const r = await apiService.get('/fin-bom/stats')
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      const r = await apiService.get('/fin-bom', {
        params: {
  ...
  useEffect(() => {
    getLovMany(['UOM', 'FIN_ITEM_TYPE', 'PURITY']).then(map => {
      setUomOpts(      (map as Record<string, UomOpt[]>).UOM            ?? [])
      setItemTypeOpts( (map as Record<string, UomOpt[]>).FIN_ITEM_TYPE  ?? [])
  ...
  useEffect(() => {
    let cancelled = false
    apiService.get('/workflow/access/FINDING_BOM').then(r => {
      if (cancelled) return
  ...
  const handleExport = async (scope: 'all' | 'selected', fmt: 'csv' | 'excel' | 'pdf') => {
    ...
    try {
      ...
      if (scope === 'all' && total > items.length) {
        const r = await apiService.get('/fin-bom', { params: { page: 1, limit: 9999, status: statusFilter } })
      ...
      const fname = `Finding_BOM_${statusFilter}_${Date.now()}`
      if (fmt === 'csv')   exportToCSV(data, fname)
      else if (fmt === 'excel') await exportToExcel(data, fname)
      else await exportToPDF(data, fname, 'Finding Bill of Materials')
  ...
  const openBOM = async (row: BOMVariantRow) => {
    ...
    setView('form')
    try {
      const r = await apiService.get(`/fin-bom/variant/${row.variant_id}`)
      const d = r.data?.data
      setHeaderInfo({
  ...
  const saveDraft = async (): Promise<boolean> => {
    ...
    try {
      ...
      if (bomId) {
        await apiService.put(`/fin-bom/${bomId}`, payload)
        toast.success('BOM saved')
      } else {
        const r = await apiService.post('/fin-bom', payload)
        ...
        <div className="flex items-center justify-between gap-2 text-xs" style={{ color: 'var(--text-muted)' }}>
          <div className="flex items-center gap-2">
            <button onClick={askBackToList} className="hover:text-[var(--accent-gold)] flex items-center gap-1">
              <ArrowLeftIcon className="w-3.5 h-3.5" /> Bill of Materials (Finding BOM)
            </button>
            <span>/</span>
