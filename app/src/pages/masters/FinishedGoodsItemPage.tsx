...
import FGImportWizard         from '@/components/masters/FGImportWizard'
...
interface DesignMaster {
  ...
  product_name: string      // Sub Collection (SUB_COLLECTION lookup)
...
interface ItemMaster {
  ...
  collection_name: string
  product_name: string     // Sub Collection, copied from the design
  product_category: string // PRODUCT lookup, picked per item
  sub_category: string
  ...
  design_no?: string
  item_image?: string
  design_image?: string  // joined from design_master in fg_item_get_by_id
  design_id?: number
...
const BOM_STATUS_META: Record<string, { label: string; variant: 'secondary' | 'warning' | 'success' | 'danger' }> = {
...
const itemSchema = yup.object({
  ...
  product_category:    yup.string().default(''),
...
const designSchema = yup.object({
  product_name:    yup.string().required('Sub Collection is required'),
...
// ── Main Page ─────────────────────────────────────────────────────────────────
const FinishedGoodsItemPage: React.FC = () => {

  // ── Permissions ─────────────────────────────────────────────
  const { canView, canCreate, canUpdate, canDelete } = usePermission('MM_FG_ITEMS')
  ...
  } = useForm<ItemFormValues>({ resolver: yupResolver(itemSchema) as never, defaultValues: { status: 'DRAFT' } })
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      const res = await dynamicApi.get('fg_item_list_get', {}, {
        page,
        limit:  pageSize,
  ...
  const loadStats = useCallback(async () => {
    try {
      const res = await dynamicApi.get('fg_item_stats')
  ...
  const loadDesigns = useCallback(async () => {
    try {
      const res = await dynamicApi.get('design_list_get', { design_type: 'FG' })
  ...
  const loadVariantClients = async (variantId: number) => {
    try {
      const res = await dynamicApi.get('fg_variant_client_get', { variant_id: variantId })
  ...
  const loadVariants = useCallback(async (itemId: number) => {
    ...
    try {
      const res = await dynamicApi.get('fg_variant_list_get', { item_id: itemId })
  ...
  const handleExport = async (scope: 'all' | 'selected', fmt: 'csv' | 'excel' | 'pdf') => {
    ...
    if (scope === 'selected') {
      ...
      setExporting(true)
      try {
        const res = await dynamicApi.get('fg_item_list_get', {}, { page: 1, limit: 9999, status: statusFilter })
        rows = res.data?.data ?? []
    ...
    else                      void exportToPDF(data, fname, 'FG Master')
  }

  const confirmToggle = async (reason?: string) => {
    if (!toggleItem) return
    try {
      await dynamicApi.put('fg_item_toggle', { id: toggleItem.id }, reason ? { reason } : {})
  ...
  const renderCell = (key: string, row: ItemMaster) => {
    switch (key) {
      ...
      case 'product_name':
        return <span className="text-sm text-[var(--text-secondary)]">{subCollectionLabel(row.product_name) || '—'}</span>
      case 'sub_category':
      ...
      case 'sku_type':
        return <span className="text-sm text-[var(--text-secondary)]">{lookupLabel('FG_SKU_TYPE', row.sku_type) || '—'}</span>
      case 'gender':
  ...
  const openAddForm = async () => {
    ...
    resetItem({ design_code: '', manufacturing_name: '', jewellery_type: '', sku_type: '', gender: '', tech_type: '', manufacturing_level: '', occasion: '', group_sales: '', sub_category: '', product_category: '', status: 'DRAFT', uom1: 'NOS', uom2: 'GRAM', video_upload: '', video_360: '' })
    setView('form')
  }
  ...
  const openEditForm = async (item: ItemMaster, mode: 'edit' | 'view' = 'edit') => {
    ...
    try {
      const [itemRes, designsRes] = await Promise.all([
        dynamicApi.get('fg_item_get_by_id', { id: item.id }),
        dynamicApi.get('design_list_get', { design_type: 'FG' }),
      ])
  ...
  const handleSaveHeader = async (data: ItemFormValues) => {
    ...
    try {
      if (formMode === 'add') {
        ...
        const res = await dynamicApi.post('fg_item_create', {}, payload)
        ...
        const res = await dynamicApi.put('fg_item_update', { id: currentItem.id }, payload)
  ...
  const openDesignDrawer = () => {
    resetDesign({ product_name: '', collection_name: '', design_no: '', design_code: '' })
  ...
  const handleSaveDesign = async (data: DesignFormValues) => {
    ...
    try {
      ...
      const payload = { ...data, design_no, design_code, design_attributes: '{}', design_type: 'FG' }
  ...
  const openAddVariant = () => {
    ...
    setVariantViewOnly(false); setEditingVariant(null); setVariantTab('sku_info')
    setClientRows([]); setClientEdit(null); setClientEditError({}); setVariantSaveError(null)
    resetVar({
      karat_color: '', sku_type: lo('FG_SKU_TYPE')[0]?.lookup_code ?? '', group_sales: '', old_erp_variant: '', weight_band: '', size: '', width_size: '', style_tone: '',
      vendor_name: '',
      ...
      rubber_die_number: '', wax_resin_weight: '',
  ...
  const openEditVariant = async (v: ItemVariant, viewOnly = false) => {
    setVariantViewOnly(viewOnly); setEditingVariant(v); setVariantTab('sku_info')
  ...
  const handleSaveVariant = async (data: VariantFormValues) => {
    ...
    try {
      ...
      if (editingVariant) {
        await dynamicApi.put('fg_variant_update', { id: editingVariant.id }, { ...payload, id: editingVariant.id })
        variantId = editingVariant.id
        toast.success('Variant updated')
      } else {
        const res = await dynamicApi.post('fg_variant_create', {}, payload)
        variantId = res.data?.data?.[0]?.id
        toast.success('Variant created')
      }

      // Save variant clients: delete all then re-insert
      await dynamicApi.delete('fg_variant_client_delete_all', { variant_id: variantId })
      await Promise.all(
        clientRows.map(({ _key: _, ...c }) =>
          dynamicApi.post('fg_variant_client_create', {}, { variant_id: variantId, ...c })
        )
      )
  ...
  const confirmDeleteVariant = async () => {
    ...
    try {
      await dynamicApi.put('fg_variant_toggle', { id: deleteVariant.id }, {})
  ...
  // Sub Collection is stored in product_name and now picked from lookup type
  // SUB_COLLECTION. Resolve against that first and fall back to PRODUCT so
  // designs created before the switch still show a name, not a raw code.
  const subCollectionLabel = (code: string) => {
    if (!code) return ''
    const opt = lo('SUB_COLLECTION').find(o => o.lookup_code === code)
             ?? lo('PRODUCT').find(o => o.lookup_code === code)
    return opt ? opt.lookup_name : code
  }
  ...
  const renderList = () => (
    <div>
      <PageBreadcrumb parent="Master Management" current="FG Master" />
                ...
                <tr key={row.id}
                  ...
                  <td className="px-4 py-2.5 sticky right-0 z-10 bg-[var(--bg-card)] border-l border-[var(--border-color)]">
                    <div className="flex items-center justify-center gap-1">
                      {canUpdate && (
                        <button onClick={() => openEditForm(row)} className={`p-1.5 rounded hover:bg-[var(--bg-tertiary)] text-blue-500 ${!row.is_active ? 'invisible' : ''}`} title="Edit">
    ...
    <div>
      ...
      <div className="flex items-center gap-2 text-xs mb-4" style={{ color: 'var(--text-muted)' }}>
        <button onClick={() => setView('list')} className="hover:text-[var(--accent-gold)] flex items-center gap-1">
          <ArrowLeftIcon className="w-3.5 h-3.5" /> FG Master
        </button>
        <span>/</span>
            ...
            <div className="flex-1 min-w-0">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {[
                  ...
                  { label: 'Sub Collection', val: subCollectionLabel((selectedDesign ?? currentItem)?.product_name ?? '') },
                ...
                {/* Product Category — PRODUCT lookup, stored on the item */}
                <div>
                  ...
                  {formMode === 'view' ? (
                    <div className="form-input cursor-not-allowed opacity-70 bg-[var(--bg-tertiary)]" style={{ minHeight: '38px' }}>
                      {lookupLabel('PRODUCT', currentItem?.product_category ?? '') || <span className="text-[var(--text-muted)]">—</span>}
                    </div>
                  ) : (
                    <Controller name="product_category" control={controlItem} render={({ field }) => (
                      <SearchSelect options={lo('PRODUCT')} value={field.value ?? ''} onChange={field.onChange} placeholder="Search product category…" />
                    )} />
                  )}
                  {itemErrors.product_category && <p className="text-xs text-red-500 mt-1">{itemErrors.product_category.message}</p>}
                </div>
        ...
        <div className="card p-5 mb-4">
          ...
          {itemTab === 'media' && (
            <div>
              ...
              <ItemMediaPanel masterType="fg" itemId={currentItem?.id} canUpload={formMode !== 'view'} />
            </div>
          )}
        </div>
        ...
        <div className="card overflow-hidden">
          ...
          {variantsLoading ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--border-color)] bg-[var(--bg-secondary)]">
                  {['SKU Code','Collection','Sub Collection','SKU Type','Karat / Color','Weight Band','Size','BOM Status','Actions'].map(h => (
              ...
              <tbody>
                {variants.map(v => (
                  <tr key={v.id} className="border-b border-[var(--border-color)] hover:bg-[var(--bg-secondary)] transition-colors">
                    ...
                    <td className="px-4 py-2.5 text-sm text-[var(--text-secondary)]">{subCollectionLabel(currentItem?.product_name ?? '') || '—'}</td>
                    <td className="px-4 py-2.5 text-sm text-[var(--text-secondary)]">{lookupLabel('FG_SKU_TYPE', v.sku_type) || v.sku_type || '—'}</td>
                    ...
                    <td className="px-4 py-2.5">
                      <Badge
                        label={(BOM_STATUS_META[v.bom_status ?? 'NO_BOM'] ?? BOM_STATUS_META.NO_BOM).label}
                        variant={(BOM_STATUS_META[v.bom_status ?? 'NO_BOM'] ?? BOM_STATUS_META.NO_BOM).variant}
                      />
                    </td>
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-1">
                        {/* Edit when No BOM, View when BOM exists */}
                        {formMode === 'edit' && !v.bom_id ? (
                        ...
                        {currentItem?.is_active && (
                          <button onClick={() => {
                            dispatch(openTab({ id: 'master-mgmt-fg-bom', title: 'Bill of Materials (FG)', path: '/master-mgmt/fg-bom' }))
                            navigate('/master-mgmt/fg-bom', { state: { openVariantId: v.id, skuCode: v.sku_code } })
                          }} className="p-1.5 rounded hover:bg-[var(--bg-tertiary)] text-[var(--accent-gold)]" title="Open BOM">
                            <TableCellsIcon className="w-4 h-4" />
                          </button>
                        )}
                        {/* Delete only when No BOM */}
                        {formMode === 'edit' && !v.bom_id && (
            ...
            <div className="px-6 py-5 overflow-y-auto flex-1">
              ...
              {variantTab === 'sku_info' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {/* Readonly: Collection, Sub Collection, Design No */}
                  {[
                    { label: 'Collection', val: lookupLabel('COLLECTION', currentItem?.collection_name ?? '') },
                    { label: 'Sub Collection', val: subCollectionLabel(currentItem?.product_name ?? '') },
                  ...
                  <div>
                    ...
                    <Controller name="weight_band" control={controlVar} render={({ field }) => (
                      <SearchSelect options={lo('WEIGHT_BAND')} value={field.value ?? ''} onChange={field.onChange} placeholder="Search weight band…" disabled={formMode === 'view' || variantViewOnly || !isNewVariant} />
                    )} />
                  ...
                  <div>
                    ...
                    <Controller name="size" control={controlVar} render={({ field }) => (
                      <SearchSelect options={lo('PRODUCT_SIZE')} value={field.value ?? ''} onChange={field.onChange} placeholder="Search size…" disabled={formMode === 'view' || variantViewOnly || !isNewVariant} />
                    )} />
              ...
              {variantTab === 'general' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>
                      SKU Type<span className="text-red-500 ml-0.5">*</span>
                    </label>
                    <Controller name="sku_type" control={controlVar} render={({ field }) => (
                      <SearchSelect options={lo('FG_SKU_TYPE')} value={field.value ?? ''} onChange={field.onChange} placeholder="Search SKU type…" disabled={formMode === 'view' || variantViewOnly} />
                    )} />
                  ...
                  <div className="lg:col-span-3 mt-2">
                    ...
                    {clientRows.length === 0 && !clientEdit && (
                      <p className="text-xs text-center py-3" style={{ color: 'var(--text-muted)' }}>
                        No client variants added. {formMode !== 'view' && 'Click "Add Client" above.'}
                      </p>
                    )}
                  </div>
                </div>
          ...
          <div className="absolute top-0 right-0 h-full flex flex-col"
            ...
            <form onSubmit={submitDesign(handleSaveDesign)} className="flex flex-col flex-1 min-h-0">
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
                ...
                {/* Sub Collection — stored in the design's product_name column */}
                <div>
                  <label className="block text-sm font-medium mb-1" style={{ color: 'var(--text-secondary)' }}>
                    Sub Collection<Req />
                  </label>
                  <Controller name="product_name" control={controlDesign} render={({ field }) => (
                    <SearchSelect options={lo('SUB_COLLECTION')} value={field.value ?? ''} onChange={field.onChange} placeholder="Search sub collection…" />
                  )} />
...
export default FinishedGoodsItemPage
