...
CREATE TABLE IF NOT EXISTS sales_order_hdr (
    ...
    order_type      VARCHAR(50)  NOT NULL,                      -- SALES_ORDER_TYPE lookup (no default — drives the item master)
...
-- Order Type drives which master the Customer Item picker searches, so each
-- code maps to one: STK/CUS → FG Master variants, FIO → Finding Master variants.
