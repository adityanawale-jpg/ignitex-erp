-- ============================================================
-- IGNITEX.AI ERP - Reset & Initialize Database
-- Version: 2.0.0
-- WARNING: Drops ALL existing tables and recreates from scratch.
--          All data will be lost. Run only on a fresh/dev DB.
-- ============================================================

BEGIN;

-- ============================================================
-- DROP ALL TABLES (child → parent order with CASCADE)
-- ============================================================
...
CREATE TABLE user_master (
    ...
    department_id    VARCHAR(50),               -- lookup_type = 'DEPARTMENT' in master_lookup
...
-- ============================================================
-- USER ROLE  (user ↔ role many-to-many; first assigned = default)
-- ============================================================
...
-- ============================================================
-- PROJECT CONFIG  (dynamic queries)
-- ============================================================
...
-- ============================================================
-- MASTER LOOKUP  (departments, party types, statuses, etc.)
-- ============================================================
...
-- Manager: all menus except user/role/config management
INSERT INTO role_menu_mapping (role_id, menu_id, can_view, can_create, can_update, can_delete)
SELECT 2, id, TRUE, TRUE, TRUE, FALSE
FROM menu_master
WHERE menu_code NOT IN ('USER_MASTER', 'ROLE_MASTER', 'SYSTEM_CONFIG');
...
('menu_get',
 'SELECT m.*, rm.can_view, rm.can_create, rm.can_update, rm.can_delete FROM menu_master m LEFT JOIN role_menu_mapping rm ON m.id = rm.menu_id AND rm.role_id = :role_id WHERE m.is_active = TRUE ORDER BY m.menu_level, m.menu_order',
...
-- Departments (used by user_master.department_id)
...
(1, 1, TRUE),   -- EMP001 → ADMIN
(2, 2, TRUE),   -- EMP002 → MANAGER
(3, 3, TRUE);   -- EMP003 → SALES
...
COMMENT ON TABLE master_lookup    IS 'Common lookup/dropdown data — includes DEPARTMENT, PARTY_TYPE, etc.';
