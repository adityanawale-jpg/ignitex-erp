-- Bifurcate designs by module (FG vs Finding) so a "New Design" created from
-- FG Master's Design Code search doesn't show up in Finding Master's, and
-- vice versa. Existing designs used by exactly one module are backfilled to
-- that type; designs already shared by both modules (or unused by either)
-- are left NULL, which design_list_get still treats as visible everywhere —
-- preserving current behavior for that ambiguous slice of historical data.
